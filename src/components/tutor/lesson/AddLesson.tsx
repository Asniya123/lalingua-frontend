...
const AddLesson: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!courseId || !title || !description || !videoFile || !introVideoFile) {
      toast.error("Please fill in all lesson fields and upload both videos.");
      return;
    }
    setLoading(true);
    ...
    try {
      ...
      const [introVideoUrl, videoUrl] = await Promise.all([
        uploadVideo(introVideoFile, "intro"),
        uploadVideo(videoFile, "lesson"),
      ]);

      if (!introVideoUrl || !videoUrl) throw new Error("Video upload failed.");
      const lessonData: ILesson = { courseId, title, description, videoUrl, introVideoUrl };
        ...
        <Card className="max-w-2xl mx-auto bg-white rounded-xl shadow-lg">
          ...
          <CardContent className="p-6 space-y-6">
            <form onSubmit={handleSubmit} className="space-y-6">
              <Input placeholder="Enter lesson title" value={title} onChange={(e) => setTitle(e.target.value)} disabled={loading} type={""} />
              <textarea
              ...
              />
              {[{ id: "introVideo", file: introVideoFile, label: "Intro Video", type: "intro" }, { id: "lessonVideo", file: videoFile, label: "Lesson Video", type: "lesson" }].map(({ id, file, label, type }) => (
              ...
              <div className="flex gap-4">
                ...
                <Button type="button" onClick={handleCancel} disabled={!loading} className="w-full bg-gray-500 hover:bg-gray-600">
                  Cancel
                </Button>
              </div>
            </form>
